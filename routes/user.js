...
router.get('/admin/user/edit/:id', common.restrict, (req, res) => {
    ...
    db.users.findOne({_id: common.getId(req.params.id)}, (err, user) => {
        ...
        if(user.userEmail !== req.session.user && req.session.isAdmin === false){
            req.session.message = 'Доступ заборонено';
        ...
        db.users.update({_id: common.getId(req.body.userId)},
            {
                ...
                if(err){
                    console.error(colors.red('Помилка оновлення користувача: ' + err));
                    req.session.message = 'Не вдалося оновити користувача';
                    req.session.messageType = 'danger';
                    res.redirect('/admin/user/edit/' + req.body.userId);
                }else{
                    // show the view
                    req.session.message = 'Обліковий запис користувача оновлено.';
                    req.session.messageType = 'success';
                    res.redirect('/admin/user/edit/' + req.body.userId);
                }
            });
    ...
    db.users.count({}, (err, userCount) => {
        ...
        db.users.findOne({'userEmail': req.body.userEmail}, (err, user) => {
            if(user){
                ...
                console.error(colors.red('Не вдалося вставити користувача, можливо, вже існує: ' + err));
            ...
            db.users.insert(doc, (err, doc) => {
                ...
                if(err){
                    if(doc){
                        console.error(colors.red('\n' + 'Не вдалося вставити користувача: ' + err));
                        req.session.message = 'Користувач існує';
                        req.session.messageType = 'danger';
                        res.redirect('/admin/user/edit/' + doc._id);
                        return;
                    }
                    console.error(colors.red('Не вдалося вставити користувача: ' + err));
                    req.session.message = 'Не вдалося створити нового користувача';
                    req.session.messageType = 'danger';
                    res.redirect('/admin/user/new');
                    return;
                }
